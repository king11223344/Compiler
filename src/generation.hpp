...
public:
    ...
    void gen_term(const NodeTerm* term)
    {
        struct TermVisitor {
            ...
            {
                const auto it = ranges::find_if(as_const(gen.m_vars), [&](const Var& var) {
    ...
    void gen_if_pred(const NodeIfPred* pred, const string& end_label)
    {
        struct PredVisitor {
            Generator& gen;
    ...
    {   
        struct StmtVisitor {
            ...
            void operator()(const NodePrintStmt* stmt_print) const
            {      
                    string existingContent = gen.m_output.str();
                    gen.m_output.str(""); 
                    gen.m_output << "section .data\n";
                    gen.m_output << "msg"<<gen.print_count<<" db '" << stmt_print->value.value.value() << "', 0\n";
                    gen.m_output << "section .text\n";
                    gen.m_output << existingContent; 
                    gen.m_output << "    mov rax, 1\n";
                    gen.m_output << "    mov rdi, 1\n";
                    gen.m_output << "    mov rsi, msg"<<gen.print_count++<<"\n";
                    gen.m_output << "    mov rdx, " << stmt_print->value.value.value().size() << "\n";
            ...
            {
                const auto it = ranges::find_if(gen.m_vars, [&](const Var& var) {
                    return var.name == stmt_assign->ident.value.value();
                });
                if (it == gen.m_vars.end()) {
                    cerr << "Undeclared identifier: " << stmt_assign->ident.value.value() << endl;
                    exit(EXIT_FAILURE);
                }
